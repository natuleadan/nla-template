...
import { getInventory, updateInventory, deleteInventory, createInventory } from "@/lib/modules/inventory"
...
export async function GET(request: Request, { params }: RouteParams) {
  try {
    ...
    if (!productSlug || typeof productSlug !== "string") return badRequest("productSlug invalido")
...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    if (!Array.isArray(body.locations)) return badRequest("El campo locations debe ser un array")

    const locations = body.locations.map((loc: unknown, idx: number) => {
      if (!loc || typeof loc !== "object") throw new Error("locations[" + idx + "] debe ser un objeto")
      const l = loc as Record<string, unknown>
      if (typeof l.location !== "string") throw new Error("locations[" + idx + "].location debe ser string")
      if (typeof l.quantity !== "number") throw new Error("locations[" + idx + "].quantity debe ser number")
      return { 
        location: l.location, 
        quantity: l.quantity, 
    ...
    return NextResponse.json({ message: msg, locations: created }, { status: 201 })
  } catch (error) {
...
export async function PUT(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const locations = body.locations.map((loc: unknown, idx: number) => {
      ...
      if (typeof l.location !== "string") throw new Error("locations[" + idx + "].location debe ser string")
      if (typeof l.quantity !== "number") throw new Error("locations[" + idx + "].quantity debe ser number")
      if (typeof l.reserved !== "number") throw new Error("locations[" + idx + "].reserved debe ser number")
      if (typeof l.available !== "number") throw new Error("locations[" + idx + "].available debe ser number")
      return { location: l.location, quantity: l.quantity, reserved: l.reserved, available: l.available }
    })
