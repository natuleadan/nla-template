...
import { validateApiKey, unauthorized, badRequest, notFound } from "@/lib/config/env"
...
export async function GET(request: Request) {
  ...
  if (!page || typeof page !== "string") return badRequest("El parámetro 'page' es requerido")
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!body.title || typeof body.title !== "string") return badRequest("El campo 'title' es requerido")
    if (!body.page || typeof body.page !== "string") return badRequest("El campo 'page' es requerido")
    ...
    return NextResponse.json({ message: "Página creada", ...newPage }, { status: 201 })
  } catch (error) {
