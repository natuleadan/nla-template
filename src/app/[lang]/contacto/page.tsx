...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: `${baseUrl}/${lang}/contacto/opengraph-image`, width: 1200, height: 630, alt: cfg.pages.contacto.title }],
    ...
    twitter: {
      ...
      title: `${cfg.pages.contacto.title} | ${brand.name}`,
      description: cfg.pages.contacto.description,
      images: [{ url: `${baseUrl}/${lang}/contacto/twitter-image`, width: 1200, height: 600, alt: cfg.pages.contacto.title }],
    },
    ...
    <>
      <JsonLdBreadcrumb
        levels={[
          ...
          { name: cfg.pages.contacto.title, item: `${getBaseUrl()}/${lang}/contacto` },
        ]}
      />
      <JsonLdContact locale={lang} />
      <div className="px-4 md:px-6 lg:px-8 max-w-7xl mx-auto w-full py-8">
        <PageHeader title={cfg.pages.contacto.title} description={cfg.pages.contacto.description} />
        <div className="grid gap-12 md:grid-cols-2">
          <div className="space-y-4">
            <h2 className="text-xl font-semibold mt-8 mb-4">{cfg.pages.contacto.findUs}</h2>
            ...
            <div className="flex items-start gap-4 py-3">
              ...
              <div>
                <p className="font-medium">{cfg.pages.contacto.emailLabel || "Email"}</p>
            ...
            <div className="flex items-start gap-4 py-3">
              ...
              <div>
                <p className="font-medium">{cfg.pages.contacto.whatsappLabel || "WhatsApp"}</p>
                <p className="text-sm text-muted-foreground">{getWhatsappNumber() || brand.phone}</p>
              </div>
            </div>
          </div>
          <div className="space-y-4">
            <h2 className="text-xl font-semibold mt-8 mb-4">{cfg.pages.contacto.writeUs}</h2>
            <ContactForm />
          </div>
        </div>
      </div>
