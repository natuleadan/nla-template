...
function getNextDateForDay(dayName: string, time: string, names: string[]): string {
  ...
  const availableSlots = days.flatMap((d) =>
    d.slots
      ...
      .map((s) => ({
        ...
        type: s.type,
        description: s.type ? cfg.agenda.jsonld.citaDe(s.type, brand.name) : cfg.agenda.jsonld.citaDisponible,
      })),
  );
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: `${baseUrl}/${lang}/agenda/opengraph-image`, width: 1200, height: 630, alt: title }],
      locale: getLocaleFromLang(lang),
    },
    twitter: {
      card: "summary_large_image", title, description,
      images: [{ url: `${baseUrl}/${lang}/agenda/twitter-image`, width: 1200, height: 600, alt: title }],
    },
...
export default async function AgendaPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return (
    <>
      <JsonLdBreadcrumb levels={[{ name: cfg.nav.items[0].label, item: `${baseUrl}/${lang}` }, { name: cfg.agenda.page.title, item: `${baseUrl}/${lang}/agenda` }]} />
      <JsonLdAgendaList name={cfg.agenda.page.title} slots={availableSlots} baseUrl={baseUrl} businessName={brand.name} />
      <div className="px-4 md:px-6 lg:px-8 max-w-7xl mx-auto w-full py-8">
        <div className="flex flex-row items-start justify-between mb-8 gap-4">
          <div className="flex-1 min-w-0">
            <PageHeader title={cfg.agenda.page.title} description={cfg.agenda.page.description} className="" />
          </div>
          <div className="shrink-0 pt-1"><CurrentTime /></div>
        </div>
