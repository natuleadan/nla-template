...
async function getInitialData(locale: string) {
  "use cache";
  cacheLife("hours");
  const [allPosts, categories] = await Promise.all([getAllPosts(locale), getBlogCategories(locale)]);
  const initial = await getPosts(1, 6, locale);
  return { posts: allPosts, categories, initialPosts: initial.posts, total: initial.total, hasMore: initial.hasMore };
}
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: `${baseUrl}/${lang}/blog/opengraph-image`, width: 1200, height: 630, alt: title }],
      locale: getLocaleFromLang(lang),
    },
    twitter: {
      card: "summary_large_image", title, description,
      images: [{ url: `${baseUrl}/${lang}/blog/twitter-image`, width: 1200, height: 600, alt: title }],
    },
...
export default async function BlogPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const cfg = getConfig(lang);
  const { categories, initialPosts, total, hasMore, posts } = await getInitialData(lang);
  const baseUrl = getBaseUrl();
  ...
  return (
    <>
      <JsonLdBreadcrumb levels={[{ name: cfg.nav.items[0].label, item: `${baseUrl}/${lang}` }, { name: cfg.blog.page.title, item: `${baseUrl}/${lang}/blog` }]} />
      <JsonLdBlogList name={cfg.blog.page.title} total={total} posts={jsonLdPosts} />
      <div className="px-4 md:px-6 lg:px-8 max-w-7xl mx-auto w-full py-8">
        <PageHeader title={cfg.blog.page.title} description={cfg.blog.page.description} />
        <Suspense fallback={<div className="space-y-6">
          ...
          {total > 0 ? (
            <BlogToolbar initialPosts={initialPosts} total={total} initialHasMore={hasMore} categories={categories} />
          ) : (
            <Empty className="py-12"><p className="text-muted-foreground">{cfg.blog.page.empty}</p></Empty>
          )}
        </Suspense>
      </div>
    </>
