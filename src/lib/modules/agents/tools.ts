...
Usa el campo "slug" exacto para llamar a getProductDetail si el usuario pide más info.`,
      inputSchema: z.object({
        ...
        category: z
          .string()
          .optional()
          .describe("Filtrar por categoría exacta"),
      }),
...
El slug debe ser exactamente el mismo que devolvió getProducts.`,
      inputSchema: z.object({
        slug: z.string().describe("Slug exacto del producto"),
      }),
      execute: async ({ slug }) => {
        const product = await getProduct(slug);
        if (!product)
          return { found: false, message: `Producto "${slug}" no encontrado` };
        const fmt = (v: number | undefined) =>
          v !== undefined ? `$${(v / 100).toFixed(2)} USD` : undefined;
        return {
          found: true,
          product: {
            name: product.name,
...
Usa getPageDetail con el slug exacto para leer el contenido completo.`,
      inputSchema: z.object({
        ...
        category: z
          .string()
          .optional()
          .describe("Filtrar por categoría (ej: legal, politicas)"),
      }),
      execute: async ({ query, category }) => {
    ...
    getPageDetail: tool({
      ...
      inputSchema: z.object({
        slug: z.string().describe("Slug exacto de la página"),
      }),
      execute: async ({ slug }) => {
        const page = await getPagina(slug);
        if (!page)
          return { found: false, message: `Página "${slug}" no encontrada` };
        return {
          found: true,
          page: {
            title: page.title,
...
Usa getPostDetail con el slug exacto para leer el artículo completo.`,
      inputSchema: z.object({
        ...
        category: z
          .string()
          .optional()
          .describe("Filtrar por categoría (ej: nutricion, entrenamiento)"),
      }),
      execute: async ({ query, category }) => {
    ...
    getPostDetail: tool({
      ...
      inputSchema: z.object({
        slug: z.string().describe("Slug exacto del artículo"),
      }),
      execute: async ({ slug }) => {
        const post = await getPost(slug);
        if (!post)
          return { found: false, message: `Artículo "${slug}" no encontrado` };
        return {
          found: true,
          post: {
            title: post.title,
    ...
    getCompanyInfo: tool({
      ...
      execute: async () => ({
        ...
        country: brand.addressCountry,
        instagram: brand.socialInstagram,
        facebook: brand.socialFacebook,
        twitter: brand.socialTwitter,
...
INPUT: key = nombre del dato, value = valor, override = sobrescribir si ya existe (default false).`,
      inputSchema: z.object({
        key: z
          .string()
          .describe("Nombre del dato (ej: alergias, nombre, talla_ropa)"),
        value: z.string().describe("Valor del dato"),
        override: z
          .boolean()
          .optional()
          .default(false)
          .describe("Sobrescribir si ya existe"),
      }),
      execute: async ({ key, value, override }) => {
        ...
        if (!override && key in existing) {
          return {
            success: false,
            message: `"${key}" ya existe: "${existing[key]}". Usa override=true.`,
    ...
    deleteMemory: tool({
      ...
      inputSchema: z.object({
        confirm: z
          .string()
          .describe(
            'Texto de confirmación. Si es "BORRAR" ejecuta, si no devuelve error.',
          ),
      }),
...
INPUT: reason = motivo de la derivación (describe qué necesita el usuario).`,
      inputSchema: z.object({
        reason: z
          .string()
          .describe("Motivo claro de la derivación: qué necesita el usuario"),
      }),
      execute: async ({ reason }) => {
        ...
        return {
          success: true,
          message: `Has sido transferido a un asesor humano. En breve te atenderemos. Motivo: ${reason}`,
        };
      },
    }),
  };
  ...
  if (context.isAdmin) {
    const adminTools: Record<string, Tool> = {
      getDerivedConversations: tool({
        description:
          "Lista todas las conversaciones derivadas a humano pendientes.",
        inputSchema: z.object({}),
        execute: async () => {
      ...
      resolveDerivation: tool({
        description:
          "Resuelve una derivación: libera el chat para que el agente vuelva a responder automáticamente.",
        inputSchema: z.object({
          phone: z
            .string()
            .describe("Número de teléfono del usuario a liberar"),
        }),
        execute: async ({ phone }) => {
          ...
          return {
            success: ok,
            message: ok
              ? `Derivación liberada para ${phone}.`
              : "No se pudo liberar.",
          };
        },
      }),
...
Devuelve: información de la sesión y el historial reciente.`,
        inputSchema: z.object({
          phone: z
            .string()
            .describe("Número de teléfono completo (ej: 593991234567)"),
        }),
        execute: async ({ phone }) => {
          const { getSession, anonymizePhone } =
            await import("@/lib/modules/agents/session-store");
          const anonymized = await anonymizePhone(phone);
          const session = await getSession(phone);
          if (!session)
            return {
              found: false,
              message: "No se encontró sesión para ese número.",
              anonymized,
            };
          return {
            found: true,
