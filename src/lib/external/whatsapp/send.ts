import { getYcloudApiKey, getWhatsappNumber, getAdminPhone, isDev } from "@/lib/env";
...
async function sendRaw(to: string, message: string): Promise<boolean> {
  ...
  try {
    await new WhatsappMessagesApi(new Configuration({ apiKey }))
...
export async function notifyCustomer(phone: string, message: string): Promise<boolean> {
